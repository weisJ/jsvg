...
public class FXAWTBridge {
    ...
    public static Affine convertAffineTransform(AffineTransform awtTransform){
        return new Affine(awtTransform.getScaleX(), awtTransform.getShearY(), awtTransform.getShearX(), awtTransform.getScaleY(), awtTransform.getTranslateX(), awtTransform.getTranslateY());
    }

    public static AffineTransform convertAffine(Affine jfxAffine){
        return new AffineTransform(jfxAffine.getMxx(), jfxAffine.getMyx(), jfxAffine.getMxy(), jfxAffine.getMyy(), jfxAffine.getTx(), jfxAffine.getTy());
    }
    ...
    public static void drawShape(@NotNull GraphicsContext ctx, @NotNull Shape shape){
        if(shape instanceof Line2D) {
            ...
            ctx.strokeRoundRect(roundRect.getX(), roundRect.getY(), roundRect.getWidth(), roundRect.getHeight(), roundRect.getArcWidth(), roundRect.getArcHeight());
        }else if(shape instanceof Ellipse2D){
            Ellipse2D ellipse = (Ellipse2D) shape;
            ctx.strokeOval(ellipse.getX(), ellipse.getY(), ellipse.getWidth(), ellipse.getHeight());
        }else if(shape instanceof Arc2D){
            Arc2D arc2D = (Arc2D) shape;
            ctx.strokeArc(arc2D.getX(), arc2D.getY(), arc2D.getWidth(), arc2D.getHeight(), arc2D.getAngleStart(), arc2D.getAngleExtent(), toArcType(arc2D.getArcType()));
            ...
            ctx.bezierCurveTo(cubic.getCtrlX1(), cubic.getCtrlY1(), cubic.getCtrlX2(), cubic.getCtrlY2(), cubic.getX2(), cubic.getY2());
            ctx.stroke();
        }else{
    ...
    public static void fillShape(@NotNull GraphicsContext ctx, @NotNull Shape shape){
        if(shape instanceof Line2D) {
            ...
            RoundRectangle2D roundRect = (RoundRectangle2D) shape;
            ctx.fillRoundRect(roundRect.getX(), roundRect.getY(), roundRect.getWidth(), roundRect.getHeight(), roundRect.getArcWidth(), roundRect.getArcHeight());
        }else if(shape instanceof Ellipse2D){
            Ellipse2D ellipse = (Ellipse2D) shape;
            ctx.fillOval(ellipse.getX(), ellipse.getY(), ellipse.getWidth(), ellipse.getHeight());
        }else if(shape instanceof Arc2D){
            Arc2D arc2D = (Arc2D) shape;
            ctx.fillArc(arc2D.getX(), arc2D.getY(), arc2D.getWidth(), arc2D.getHeight(), arc2D.getAngleStart(), arc2D.getAngleExtent(), toArcType(arc2D.getArcType()));
        }else if(shape instanceof QuadCurve2D){
            QuadCurve2D quad = (QuadCurve2D) shape;
            ctx.beginPath();
            ctx.moveTo(quad.getX1(), quad.getY1());
    ...
    public static void setTransform(@NotNull GraphicsContext ctx, @NotNull AffineTransform awtTransform){
        ctx.setTransform(awtTransform.getScaleX(), awtTransform.getShearY(), awtTransform.getShearX(), awtTransform.getScaleY(), awtTransform.getTranslateX(), awtTransform.getTranslateY());
    }

    public static void applyTransform(@NotNull GraphicsContext ctx, @NotNull AffineTransform awtTransform){
        ctx.transform(awtTransform.getScaleX(), awtTransform.getShearY(), awtTransform.getShearX(), awtTransform.getScaleY(), awtTransform.getTranslateX(), awtTransform.getTranslateY());
    }
